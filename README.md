...
- **🆕 配信サービス表示** — どの配信サービス（Netflix / Crunchyroll / Hulu 等）で見られるかを情報カードに表示し、右クリックから各サービスの作品検索へ。出典は AniList の配信リンク。
...
| `au` | 原作者 |
| `nr` / `kk` | なろう発 / カクヨム発フラグ |
| `sv` | 配信サービス短縮コード配列（例 `["cr","nf"]`。`cr`=Crunchyroll / `nf`=Netflix / `prime`=Prime Video / `hulu` / `bili`=Bilibili / `hidive` / `disney` 等。AniList の配信リンク由来） |
