...
## 📋 プロジェクト管理
- [x] **Notion 連携** — 「やったこと / やりたいこと」を Notion 既存の「日記」アーカイブに
      日付ページ（日記スタイル）で記録。今回ぶんは「6/16」ページに起こした。
      今後も区切りごとに日記として追記していく方針。

## 🚧 機能の残タスク（2026-07-10 棚卸し・着手順）
1. [x] **「アニメ9選」ビュー＋画像書き出し** — 視聴済みから9作品を選び 3×3 グリッド表示、
       タイトル付き JPEG を書き出し（参考: my9games）。枠同士の入れ替え対応。✅ 2026-07-10 実装
2. [ ] **R-18作品一覧** — 別HTMLで分離管理（AniList `isAdult` フラグから取得）
3. [x] **裏コマンド／開発者モード** — 検索欄に合言葉 `animemo-dev` を入力するとON/OFFトグル
...
> ✅ 解決済み（旧メモにあった項目）: 視聴統計の既存記録反映バグ・話数範囲記録のマウス操作化
> （コミット 48d099d で対応済み）
